...
class MoleculeDataset(InMemoryDataset):
    ...
    def process(self):
        ...
        if self.dataset == "zinc_standard_agent":
            ...
            print(f"This Dataset contains {len(smiles_list)} molecules.")
            # add progress bar
            for i in tqdm(range(len(smiles_list)), desc=f"Processing {self.dataset}'s smiles: ", ncols=40, leave=True):
                print(f"Processing {i}th molecule in zinc dataset") if i % 10000 == 0 else None
            ...
            for d_path in downstream_dir:
                ...
                _, _, _, (train_smiles, valid_smiles, test_smiles) = scaffold_split(
                    downstream_dataset, downstream_smiles, task_idx=None, null_value=0, frac_train=0.8, frac_valid=0.1, frac_test=0.1, return_smiles=True
                )
                ...
                for smiles in remove_smiles:
                    ...
                    for smiles in species_list:  # record inchi for all species, not just
                        # largest (by default in create_standardized_mol_id if input has
                        # multiple species)
                        inchi = create_standardized_mol_id(smiles)
                        downstream_inchis.append(inchi)
                downstream_inchi_set.update(downstream_inchis)

            smiles_list, rdkit_mol_objs, folds, labels = _load_chembl_with_labels_dataset(os.path.join(self.root, "raw"))
            ...
            smiles_list, rdkit_mol_objs, labels = _load_pcba_dataset(self.raw_paths[0])
            downstream_inchi = set(pd.read_csv(os.path.join(self.root, "downstream_mol_inchi_may_24_2019"), sep=",", header=None)[0])
            for i in range(len(smiles_list)):
                print(i)
...
def merge_dataset_objs(dataset_1, dataset_2):
    ...
    new_dataset = MoleculeDataset(root="dataset/chembl_with_labels", dataset="chembl_with_labels", empty=True)
    # collate manually
    ...
    def _process(self):
        ...
        if self.dataset == "chembl_with_labels":
            ...
            for i in range(len(rdkit_mol_objs)):
                ...
                if rdkit_mol != None:
                    # # convert aromatic bonds to double bonds
                    fp_arr = create_circular_fingerprint(rdkit_mol, self.radius, self.size, self.chirality)
...
def create_all_datasets():
    #### create dataset
    downstream_dir = ["bace", "bbbp", "clintox", "esol", "freesolv", "hiv", "lipophilicity", "muv", "sider", "tox21", "toxcast"]
