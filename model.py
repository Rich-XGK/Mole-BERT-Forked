...
class GINConv(MessagePassing):
    ...
    def __init__(self, emb_dim, out_dim, aggr="add", **kwargs):
        ...
        self.edge_embedding01 = torch.nn.Embedding(NUM_BOND_TYPE, emb_dim)  # edge type embedding layer
        self.edge_embedding02 = torch.nn.Embedding(NUM_BOND_DIRECTION, emb_dim)  # edge direction embedding layer

        torch.nn.init.xavier_uniform_(self.edge_embedding01.weight.data)
        torch.nn.init.xavier_uniform_(self.edge_embedding02.weight.data)
    ...
    def forward(self, x, edge_index, edge_attr):
        ...
        # edge_attr.shape = [num_edges + num_nodes, 2]

        # compute edge embeddings: edge type embedding + edge direction embedding
        edge_embeddings = self.edge_embedding01(edge_attr[:, 0]) + self.edge_embedding02(edge_attr[:, 1])
        ...
        h = self.propagate(edge_index, x=x, edge_attr=edge_embeddings)
        return h
    ...
    def update(self, aggr_out):
        # aggr_out.shape = [num_nodes, emb_dim]
...
class GNN(torch.nn.Module):
    ...
    def __init__(self, num_layer, emb_dim, JK="last", drop_ratio=0, gnn_type="gin"):
        ...
        self.x_embedding01 = torch.nn.Embedding(NUM_ATOM_TYPE, emb_dim)  # atom type embedding layer
        self.x_embedding02 = torch.nn.Embedding(NUM_CHIRALITY_TAG, emb_dim)  # chirality embedding layer

        torch.nn.init.xavier_uniform_(self.x_embedding01.weight.data)
        torch.nn.init.xavier_uniform_(self.x_embedding02.weight.data)

        # List of GNN layers
        self.gnn_layers = torch.nn.ModuleList()
        for _ in range(num_layer):
            if gnn_type == "gin":
                self.gnn_layers.append(GINConv(emb_dim, emb_dim, aggr="add"))
            elif gnn_type == "gcn":
                self.gnn_layers.append(GCNConv(emb_dim))
            elif gnn_type == "gat":
                self.gnn_layers.append(GATConv(emb_dim))
            elif gnn_type == "graphsage":
                self.gnn_layers.append(GraphSAGEConv(emb_dim))
    ...
    def forward(self, *argv):
        # get node_attr, edge_index, edge_attr from argv
        # node_attr: [num_nodes, 2], edge_attr: [num_edges, 2], edge_index: [2, num_edges],
        if len(argv) == 3:
            node_attr, edge_index, edge_attr = argv[0], argv[1], argv[2]
        elif len(argv) == 1:
            data = argv[0]
        ...
        node_attr = self.x_embedding01(node_attr[:, 0]) + self.x_embedding02(node_attr[:, 1])
        h_list = [node_attr]  # list of hidden representation at each layer (including input)

        for layer in range(self.num_layer):
            h = self.gnn_layers[layer](h_list[layer], edge_index, edge_attr)
            ...
            h_list.append(h)  # add the hidden representation at each layer into the list
...
class DiscreteGNN(torch.nn.Module):
    ...
    def __init__(self, num_layer, emb_dim, num_tokens, JK="last", temperature=0.9, drop_ratio=0, gnn_type="gin"):
